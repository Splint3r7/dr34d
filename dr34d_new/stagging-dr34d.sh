...
ALIVE-HOST(){
	bash ~/tools/alive-host/alive.sh ~/tools/dr34d/Output/$IP/$IP-domains/$IP-all-subs
}
...
WAYBACK2(){
	echo -e "${YELLO}Scrapping wayback URLS of all Subdomains!${NC}"
	outfile=$IP-all-subs-alive.txt
	while read subs; do
		wget -qO- "http://web.archive.org/cdx/search/cdx?url=$subs/*&output=json&fl=original&collapse=urlkey&filter=statuscode:200" |sort |uniq |cut -d "'" -f2|cut -d "\"" -f2 >> $outfile-wayback.txt
	done < $IP-all-subs-alive.txt
