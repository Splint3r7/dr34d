...
DIRSCRAPPER(){
	#FOR MAC OS
	python /Users/hassankhan/tools/dirscraper/dirscraper.py -u https://${IP} -d >> Output/$IP/wayback-output/$IP-wayback-$(date +"%Y-%m-%d").txt
	python /Users/hassankhan/tools/dirscraper/dirscraper.py -u ${IP} -d >> Output/$IP/wayback-output/$IP-wayback-$(date +"%Y-%m-%d").txt
...
ALIENVAULT(){
	curl -s "https://otx.alienvault.com/api/v1/indicators/hostname/*.${IP}/url_list?page=00" | jq -r . | grep "\"url\"" | awk '{print $2}' | cut -d "\"" -f2 | sort -u >> Output/$IP/wayback-output/$IP-wayback-$(date +"%Y-%m-%d").txt
}
...
TRUFFLEHOG(){
	python /Users/hassankhan/tools/truffle-modified/git-scrapte.py ${organization} ${output}.txt
	
}


if [[ $1 == '--truffle' ]] || [[ $1 == "-th" ]]; then
...
#python3 ~/tools/dirsearch/dirsearch.py -x 500,501,502,503,504,505,404,301,302 -u https://account.jetblack.com -e php,asp,aspx,jsp,js,ini,html,log,txt,sql,zip,conf,cgi,json,jar,dll,xml -w ~/tools/dirsearch/wordlists/dirsearchall3.txt --plain-text-report=/root/tools/dirsearch/dir_results/http.txt --threads 25 --timeout=7 --request-by-hostname
